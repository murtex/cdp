function [st, freqs] = fst( ts, rate, lofreq, hifreq, nfreqs )
% stockwell transform signal
%
% [st, freqs] = SFT( ts, rate, lofreq, hifreq, nfreqs )
%
% INPUT
...
% implement vectorized version (via gaussian window matrix)
	...
	ts = fft( real( ts ) );
	h = [1; 2*ones(fix( (tslen-1)/2 ), 1); ones(1-rem( tslen, 2 ), 1); zeros(fix( (tslen-1)/2 ), 1)];
	ts = ts .* h;
	ts = ifft( ts );
