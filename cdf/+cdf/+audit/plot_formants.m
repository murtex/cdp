...
% callback : button down event dispatcher [function, argument] (vector cell)
	...
	function plot_marks( flegend ) % formant onsets
		h1 = gscatter( (resplab.f0(1) - trial.range(1)) * 1000, resplab.f0(2), ... % manual
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'warm', +2 ), 'MarkerEdgeColor', style.color( 'warm', -2 ), ...
			'DisplayName', 'manual' );
		gscatter( (resplab.f1(1) - trial.range(1)) * 1000, resplab.f1(2), ...
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'warm', +2 ), 'MarkerEdgeColor', style.color( 'warm', -2 ) );
		gscatter( (resplab.f2(1) - trial.range(1)) * 1000, resplab.f2(2), ...
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'warm', +2 ), 'MarkerEdgeColor', style.color( 'warm', -2 ) );
		gscatter( (resplab.f3(1) - trial.range(1)) * 1000, resplab.f3(2), ...
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'warm', +2 ), 'MarkerEdgeColor', style.color( 'warm', -2 ) );

		h2 = gscatter( (respdet.f0(1) - trial.range(1)) * 1000, respdet.f0(2), ... % detected
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'signal', +2 ), 'MarkerEdgeColor', style.color( 'signal', -2 ), ...
			'DisplayName', 'detected' );
		gscatter( (respdet.f1(1) - trial.range(1)) * 1000, respdet.f1(2), ...
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'signal', +2 ), 'MarkerEdgeColor', style.color( 'signal', -2 ) );
		gscatter( (respdet.f2(1) - trial.range(1)) * 1000, respdet.f2(2), ...
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'signal', +2 ), 'MarkerEdgeColor', style.color( 'signal', -2 ) );
		gscatter( (respdet.f3(1) - trial.range(1)) * 1000, respdet.f3(2), ...
			'ButtonDownFcn', callback, ...
			'MarkerFaceColor', style.color( 'signal', +2 ), 'MarkerEdgeColor', style.color( 'signal', -2 ) );
