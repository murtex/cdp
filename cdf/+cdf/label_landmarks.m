...
% cfg : framework configuration (scalar object)
	...
	while ~done
		...
		det1r = dsp.sec2smp( resp.range(1) + cfg.lab_landmarks_det1, run.audiorate ) + [1, 0];
		det2r = dsp.sec2smp( resp.range(2) + cfg.lab_landmarks_det2, run.audiorate ) + [1, 0];
		...
		plot( (resp.range(1) * [1, 1] - trial.range(1)) * 1000, ovryl, ... % landmarks, TODO
		...
		if det1f % detail #1
			...
			xlim( (resp.range(1) + cfg.lab_landmarks_det1 - trial.range(1)) * 1000 );
			ylim( det1yl );

			plot( (resp.range(1) * [1, 1] - trial.range(1)) * 1000, det1yl, ... % landmarks, TODO
				'ButtonDownFcn', {@fig_dispatch, 'buttondown'}, ...
				'Color', style.color( 'signal', +1 ) );
		...
		if det2f % detail #2
			...
			xlim( (resp.range(2) + cfg.lab_landmarks_det2 - trial.range(1)) * 1000 );
			ylim( det2yl );

			plot( (resp.range(1) * [1, 1] - trial.range(1)) * 1000, det2yl, ... % landmarks, TODO
				'ButtonDownFcn', {@fig_dispatch, 'buttondown'}, ...
				'Color', style.color( 'signal', +1 ) );
