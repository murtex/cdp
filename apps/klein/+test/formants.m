function debug_formants( indir, outdir, ids, ntrials, seed )
% debug formants detection
% 
% DEBUG_FORMANTS( indir, outdir, ids, ntrials, seed )
%
% INPUT
...
% seed : randomization seed (scalar numeric)
	...
	logger = xis.hLogger.instance( fullfile( outdir, sprintf( 'debug_formants_%d-%d.log', min( ids ), max( ids ) ) ) );
	logger.tab( 'debug formants detection...' );
	...
	for i = ids
		...
		read_audio( run, run.audiofile, true );
			...
			% plot detection statistics, TODO
		cdf.plot.formants( run, cfg, fullfile( outdir, sprintf( 'run_%d_formants.png', i ) ) );
		...
		rng( rs ); % pop randomness
			...
			% plot trials
		for j = trialids
			cdf.plot.trial_formants( run, cfg, j, ...
				fullfile( plotdir, sprintf( 'run_%d_trial_%d_formants.png', i, j ) ) );
		end
