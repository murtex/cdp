function tree = split_v2( tree, features, labels, nclasses, curnode )
...
% tree : tree (scalar struct)
	...
	if ~isempty( si )
		...
		if nlsamples > 0
			...
			tree = brf.split_v2( tree, features(lsamples, :), labels(lsamples), nclasses, tree.lefts(curnode) ); % recursion
		...
		if nrsamples > 0
			...
			tree = brf.split_v2( tree, features(rsamples, :), labels(rsamples), nclasses, tree.rights(curnode) ); % recursion
