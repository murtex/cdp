function landmark9( run, cfg )
% detect landmarks (version #9)
%
% LANDMARK9( run, cfg )
%
% INPUT
...
% rescaled powers
% epsilon powers
% looser plosion range restriction
% more advanced successive thresholding
	...
	for i = 1:n
		...
		if ~isempty( pairind )
			...
			% get plosion indices
		resppiser = respser; % restrict range
		if ~isnan( trial.detected.vo ) && ~isnan( trial.detected.vr )
			resppiser(trial.detected.vo + round( (trial.detected.vr-trial.detected.vo)/2 ) - refrange(1)+1:end) = [];
		end
