...
% more advanced successive thresholding
	...
	for i = 1:n
		...
		resppow = resppow(1:size( respser, 1 ));
			...
			% get ror and peaks
		cfg.glottis_rorpeak = 6; % TODO: hard-coded value!
		cfg.schwa_power = -18;
        ...
        rlen = refrange(2)-refrange(1);
        
        while numel( respglottis ) > 1 && respglottis(2) < rlen/3 % skip possible burst-transitions, TODO: hard-coded threshold
            respglottis(1:2) = [];
        end
        
        while numel( respglottis ) > 1 && respglottis(end-1) > rlen*8/9 % skip possible expirations, TODO: hard-coded threshold
            respglottis(end-1:end) = [];
        end
        
        if numel( respglottis ) > 1 % merge remaining pairs
            respglottis(2:end-1) = [];
        end
